#!/bin/bash
# Resilient Prima UCSF-PDGM extraction with auto-restart on crash
...
while true; do
    ...
    done_count=$(find /home/xuewei/MRI/UCSF_PDGM_flat_prima/ -name "*_features.pt" 2>/dev/null | wc -l)
    echo "$(date): Starting Prima UCSF-PDGM (attempt $((restarts+1))). Completed so far: $done_count/$TOTAL"
    ...
    /home/xuewei/MRI/Prima/.venv/bin/python end-to-end_inference_pipeline/extract_features.py \
        --config configs/ucsf_pdgm_extraction.yaml \
        --study_root /home/xuewei/MRI/UCSF_PDGM_flat 2>&1

    exit_code=$?
    restarts=$((restarts + 1))
    done_after=$(find /home/xuewei/MRI/UCSF_PDGM_flat_prima/ -name "*_features.pt" 2>/dev/null | wc -l)
    ...
    if [ "$exit_code" -eq 0 ]; then
        ...
        for d in /home/xuewei/MRI/UCSF_PDGM_flat_prima/*/; do
            case_id=$(basename "$d")
